...
int rectDraw::Render(
	const rectDraw::Vertex* Vertices, unsigned int VerticesSize,
...
#pragma region BindShaderParameters
	...
	glVertexAttribPointer(
		vPos_Idx,
		sizeof(rectDraw::Vector) / sizeof(float),
		GL_FLOAT, GL_FALSE,
		sizeof(rectDraw::Vertex),
		(void*)offsetof(rectDraw::Vertex, pos));

	const unsigned int vColor_Idx = glGetAttribLocation(shader, "vCol");
	glEnableVertexAttribArray(vColor_Idx);
	glVertexAttribPointer(
		vColor_Idx,
		sizeof(rectDraw::Color) / sizeof(float),
		GL_FLOAT, GL_FALSE,
		sizeof(rectDraw::Vertex),
		(void*)offsetof(rectDraw::Vertex, color));
